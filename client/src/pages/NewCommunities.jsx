    ...
    <>
      ...
      <div  className="mt-20">
        <div className="flex flex-row flex-wrap justify-center  mx-4 my-4 ">
          {houses.length > 1 &&
            houses.map((house, index) => house.tags[2]=="New Home Communities"?(
              <div
                key={index}
                className="flex flex-col justify-center shadow-2xl my-4 mx-4 rounded-2xl w-96 bg-[#ffffff] shadow-xl"
              >
                 <span className="absolute bg-red-600 w-38 text-xs text-white  text-sm rounded-xl px-4 py-1 -mt-96 -my-8 ml-2">
              {" "}
              {house.tags[2]}
            </span>{" "}
                <img
