     ...
     <div className="flex flex-row items-center justify-evenly ">
      <div className="mt-20">
        <div className="flex flex-row flex-wrap justify-center  mx-4 my-4">
          {houses.length > 1 &&
            houses.map((house, index) => house.tags[2]=="New Listings"?(
              <div
                key={index}
                className="flex flex-col justify-center shadow-2xl mt-6 mx-4 rounded-2xl w-96 bg-[#ffffff] shadow-xl"
              >
                
            <span className="absolute bg-red-600 w-32 text-xs text-white text-sm rounded-xl px-4 py-1 -mt-96 -my-4 m-4 ml-2">
              {" "}
              {house.tags[2]}
            </span>{" "}
